...
test('Show mapped fields in the fragment', async ({
	...
	await test.step('Check Data Set List is present', async () => {
		...
		await expect(firstListItem.locator('.list-group-title')).toContainText(
			dataSetLabel
		);

		await expect(firstListItem.locator('.list-group-text')).not.toBeEmpty();
	});
	...
	await test.step('Data Set request URL contains additionalAPIURLParameters and nestedFields', async () => {
		...
		const nestedFieldsValue = 'nestedFields=dataSetToDataSetTableSections';
