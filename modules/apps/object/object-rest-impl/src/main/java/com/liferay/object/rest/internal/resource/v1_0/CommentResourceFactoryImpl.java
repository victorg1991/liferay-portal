...
public class CommentResourceFactoryImpl implements CommentResource.Factory {
	...
	public CommentResourceFactoryImpl(
		CompanyLocalService companyLocalService,
		Supplier<CommentResourceImpl> commentResourceImplSupplier,
		...
		_commentResourceImplSupplier = commentResourceImplSupplier;
	...
	public CommentResource.Builder create() {
		return new CommentResource.Builder() {
			...
			public CommentResource build() {
				...
				Function<InvocationHandler, CommentResource>
					commentResourceProxyProviderFunction =
						CommentResourceFactoryImpl.
							ResourceProxyProviderFunctionHolder._commentResourceProxyProviderFunction;
			...
			@Override
			public CommentResource.Builder uriInfo(UriInfo uriInfo) {
				_uriInfo = uriInfo;
	...
	private Object _invoke(
		...
		CommentResourceImpl commentResourceImpl =
			_commentResourceImplSupplier.get();

		commentResourceImpl.setContextAcceptLanguage(
			new CommentResourceFactoryImpl.AcceptLanguageImpl(
				httpServletRequest, preferredLocale, user));

		commentResourceImpl.setContextCompany(
			_companyLocalService.getCompany(user.getCompanyId()));

		commentResourceImpl.setContextHttpServletRequest(httpServletRequest);
		commentResourceImpl.setContextHttpServletResponse(httpServletResponse);
		commentResourceImpl.setContextUriInfo(uriInfo);
		commentResourceImpl.setContextUser(user);
		commentResourceImpl.setExpressionConvert(_expressionConvert);
		commentResourceImpl.setFilterParserProvider(_filterParserProvider);
		commentResourceImpl.setGroupLocalService(_groupLocalService);
		commentResourceImpl.setResourceActionLocalService(
			_resourceActionLocalService);
		commentResourceImpl.setResourcePermissionLocalService(
			_resourcePermissionLocalService);
		commentResourceImpl.setRoleLocalService(_roleLocalService);
		commentResourceImpl.setSortParserProvider(_sortParserProvider);

		try {
			return method.invoke(commentResourceImpl, arguments);
		}
	...
	private final Supplier<CommentResourceImpl> _commentResourceImplSupplier;
