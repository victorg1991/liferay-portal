...
public class DocumentDataDefinitionType implements Serializable {
	...
	public static DocumentDataDefinitionType unsafeToDTO(String json) {
	...
	@GraphQLField(description = "The document data definition type's creator.")
	@JsonProperty(access = JsonProperty.Access.READ_ONLY)
	...
	public Long[] getDocumentMetadataSetIds() {
		if (_documentMetadataSetIdsSupplier != null) {
			documentMetadataSetIds = _documentMetadataSetIdsSupplier.get();

			_documentMetadataSetIdsSupplier = null;
		}

		return documentMetadataSetIds;
	}

	public void setDocumentMetadataSetIds(Long[] documentMetadataSetIds) {
		this.documentMetadataSetIds = documentMetadataSetIds;

		_documentMetadataSetIdsSupplier = null;
	}

	@JsonIgnore
	public void setDocumentMetadataSetIds(
		UnsafeSupplier<Long[], Exception>
			documentMetadataSetIdsUnsafeSupplier) {

		_documentMetadataSetIdsSupplier = () -> {
			try {
				return documentMetadataSetIdsUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
	...
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	protected Long[] documentMetadataSetIds;
	...
	@GraphQLField(description = "The document data definition type's ID.")
	@JsonProperty(access = JsonProperty.Access.READ_ONLY)
	protected Long id;
	...
	@GraphQLField(description = "The document data definition type's name.")
	@JsonProperty(access = JsonProperty.Access.READ_WRITE)
	@NotEmpty
	...
	public boolean equals(Object object) {
		...
		if (!(object instanceof DocumentDataDefinitionType)) {
			return false;
		}

		DocumentDataDefinitionType documentDataDefinitionType =
			(DocumentDataDefinitionType)object;

		return Objects.equals(
			toString(), documentDataDefinitionType.toString());
	}
	...
	public String toString() {
		...
		Long[] documentMetadataSetIds = getDocumentMetadataSetIds();

		if (documentMetadataSetIds != null) {
			if (sb.length() > 1) {
				sb.append(", ");
			}
			...
			for (int i = 0; i < documentMetadataSetIds.length; i++) {
				sb.append(documentMetadataSetIds[i]);

				if ((i + 1) < documentMetadataSetIds.length) {
					sb.append(", ");
				}
			}
...
// LIFERAY-REST-BUILDER-HASH:-92099579