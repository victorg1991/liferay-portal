...
export default function TemplateModal({
	...
	const hasMultipleItemTypes = itemTypes.length > 1;

	const [name, setName] = useState('');
	const [itemType, setItemType] = useState(
		hasMultipleItemTypes ? null : itemTypes[0]
	);
	const [itemSubtype, setItemSubtype] = useState(
		itemType?.subtypes.length ? itemType.subtypes[0].value : null
	);
	...
	const handleSubmit = (event) => {
		...
		const body = {
			...
			name,
			...itemType.additionalFields,
		};

		addTemplate({addTemplateURL, body, namespace})
						...
						>
							<ClaySelect
								...
								onChange={(event) => {
									...
									setItemSubtype(
										itemType?.subtypes.length
											? itemType.subtypes[0].value
											: null
									);
								}}
							>
