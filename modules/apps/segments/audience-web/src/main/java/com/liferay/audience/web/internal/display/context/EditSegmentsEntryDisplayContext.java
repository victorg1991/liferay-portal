...
import com.liferay.audience.web.internal.security.permission.resource.SegmentsEntryPermission;
...
public class EditSegmentsEntryDisplayContext {
	...
	private JSONArray _getContributorsJSONArray() throws Exception {
		...
		for (SegmentsCriteriaContributor segmentsCriteriaContributor :
			...
			if (!_CONTEXT_CONTRIBUTOR_KEY.equals(
	...
	private JSONArray _getPropertyGroupsJSONArray() throws Exception {
		...
		for (SegmentsCriteriaContributor segmentsCriteriaContributor :
			...
			JSONArray filteredPropertiesJSONArray =
			...
			for (int i = 0; i < propertiesJSONArray.length(); i++) {
				...
				if (_ALLOWED_FIELD_NAMES.contains(
						propertyJSONObject.getString("name"))) {

					filteredPropertiesJSONArray.put(propertyJSONObject);
				}
			}
			...
			jsonContributorsJSONArray.put(
				JSONUtil.put(
					...
					"properties", filteredPropertiesJSONArray
				).put(
					"propertyKey", segmentsCriteriaContributor.getKey()
				));
		}
	...
	private Map<String, Object> _getProps() throws Exception {
		return HashMapBuilder.<String, Object>put(
			...
			"segmentsConfigurationURL", StringPool.BLANK
		).put(
	...
	private static final Set<String> _ALLOWED_FIELD_NAMES = SetUtil.fromArray(
		"browser", "cookies", "hostname", "languageId", "localDate",
		"referrerURL", "requestParameters", "timezone", "url");

	private static final String _CONTEXT_CONTRIBUTOR_KEY = "context";
