/**
 * SPDX-FileCopyrightText: (c) 2025 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

package com.liferay.bulk.rest.client.dto.v1_0;

import com.liferay.bulk.rest.client.serdes.v1_0.ResetPermissionBulkActionSerDes;

import jakarta.annotation.Generated;

import java.io.Serializable;

import java.util.Objects;

/**
 * @author Alejandro Tardín
 * @generated
 */
@Generated("")
public class ResetPermissionBulkAction
	extends BulkAction implements Cloneable, Serializable {

	public static ResetPermissionBulkAction toDTO(String json) {
		return ResetPermissionBulkActionSerDes.toDTO(json);
	}

	@Override
	public ResetPermissionBulkAction clone() throws CloneNotSupportedException {
		return (ResetPermissionBulkAction)super.clone();
	}

	@Override
	public boolean equals(Object object) {
		if (this == object) {
			return true;
		}

		if (!(object instanceof ResetPermissionBulkAction)) {
			return false;
		}

		ResetPermissionBulkAction resetPermissionBulkAction =
			(ResetPermissionBulkAction)object;

		return Objects.equals(toString(), resetPermissionBulkAction.toString());
	}

	@Override
	public int hashCode() {
		String string = toString();

		return string.hashCode();
	}

	public String toString() {
		return ResetPermissionBulkActionSerDes.toJSON(this);
	}

}