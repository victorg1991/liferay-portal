...
function ImportResults({fileName, importResults}: Props) {
    ...
    const [expanded, setExpanded] = useState(!importResults["imported-draft"] && !importResults.invalid);
            ...
            {importResults.imported && (
                <ClayLayout.Sheet size='lg'>
                        <ClayPanel
                            ...
                            displayTitle={
                                <ClayPanel.Title>
                                    <ClayIcon className="text-4 text-success"symbol="check-circle-full" />
            ...
            {importResults["imported-draft"] && (
                <ClayLayout.Sheet size='lg'>
                    <ClayPanel
                        displayTitle={
                            <ClayPanel.Title>
                                <ClayIcon className="text-4 text-warning"symbol="warning-full" />
                    ...
                    >
                        <ClayPanel.Body>
                            <ul className="list-group sidebar-list-group">
                                {importResults["imported-draft"].map((result, index) => (
                                    <li
                                        className="list-group-item list-group-item-flex p-0"
                                        key={index}
                                    >
            ...
            {importResults.invalid && (
                <ClayLayout.Sheet size='lg'>
                    <ClayPanel
                        displayTitle={
                            <ClayPanel.Title>
                                <ClayIcon className="text-4 text-danger" symbol="exclamation-full" />
                                    ...
                                    >
                                        <ClayLayout.ContentCol expand>
                                            ...
                                            <div className="list-group-subtext text-danger">
                                                {result.message ? result.message : Liferay.Language.get('the-definition-of-the-item-is-invalid')}
                                            </div>
                                        </ClayLayout.ContentCol>
